...
class _CandidateDashboardState extends State<CandidateDashboard> {
  ...
  String get _candidateFiliere => candidatData['filiere'] ?? candidatData['filiere_specialite'] ?? 'Non spécifié';
  String get _candidateSexe => candidatData['sexe'] ?? candidatData['genre'] ?? 'Non spécifié';
  String get _candidateAge => candidatData['age'] ?? 'Non spécifié';
  String get _candidateDomicile => candidatData['domicile'] ?? candidatData['villeLieu'] ?? 'Non spécifié';
  String get _candidatePhotoUrl => candidatData['photo'] ?? candidatData['photoUrl'] ?? '';
  String get _candidateCvUrl => candidatData['cvUrl'] ?? '';
  String get _candidateCnibRectoUrl => candidatData['cnibRectoUrl'] ?? '';
  String get _candidateCnibVersoUrl => candidatData['cnibVersoUrl'] ?? '';
  ...
  Future<void> _loadCurrentProfile() async {
    ...
    setState(() {
      candidatData.addAll({
        ...
        'filiere_specialite': user['filiere_specialite']?.toString() ?? candidatData['filiere_specialite'] ?? '',
        'age': user['age']?.toString() ?? candidatData['age'] ?? '',
        'domicile': (user['domicile'] ?? user['villeLieu'])?.toString() ?? candidatData['domicile'] ?? '',
        'villeLieu': user['villeLieu']?.toString() ?? candidatData['villeLieu'] ?? '',
        'sexe': (user['sexe'] ?? user['genre'])?.toString() ?? candidatData['sexe'] ?? '',
        'genre': user['genre']?.toString() ?? candidatData['genre'] ?? '',
        'photo': user['photo']?.toString() ?? candidatData['photo'] ?? '',
        'photoUrl': user['photoUrl']?.toString() ?? candidatData['photoUrl'] ?? '',
  ...
  Future<void> _openRemoteDocument(String url, String title) async {
    if (url.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    if (ext == 'doc' || ext == 'docx') {
      ...
      remoteUrl = 'https://docs.google.com/gview?embedded=true&url=$encoded';
    }
        ...
        builder: (context) => Scaffold(
          ...
          body: WebViewWidget(
            controller: WebViewController()
              ...
              ..loadRequest(Uri.parse(remoteUrl)),
          ),
        ),
      ),
    );
        ...
        child: Column(
          ...
          children: [
            ...
            Expanded(
              child: ListView(
                children: [
                  _buildProfileRow("Nom Complet", candidatData['nom'] ?? 'Non spécifié'),
                  _buildProfileRow("Téléphone", candidatData['telephone'] ?? 'Non spécifié'),
                  _buildProfileRow("Adresse Email", candidatData['email'] ?? 'Non spécifié'),
                  ...
                  if (_candidateCvUrl.isNotEmpty)
                    Padding(
                      padding: const EdgeInsets.only(top: 10.0),
                      child: TextButton.icon(
                        onPressed: () => _openRemoteDocument(_candidateCvUrl, 'Mon CV'),
                        icon: const Icon(Icons.open_in_new, color: Colors.blue),
                        label: Text(
                          'Voir mon CV enregistré',
                  ...
                  if (_candidateCnibRectoUrl.isNotEmpty)
                    Padding(
                      padding: const EdgeInsets.only(top: 10.0),
                      child: TextButton.icon(
                        onPressed: () => _openRemoteDocument(_candidateCnibRectoUrl, 'CNIB Recto'),
                        icon: const Icon(Icons.open_in_new, color: Colors.blue),
                        label: Text(
                          'Voir le recto CNIB enregistré',
                          style: TextStyle(color: Colors.blue[900]),
                        ),
                      ),
                    ),
                  ...
                  if (_candidateCnibVersoUrl.isNotEmpty)
                    Padding(
                      padding: const EdgeInsets.only(top: 10.0),
                      child: TextButton.icon(
                        onPressed: () => _openRemoteDocument(_candidateCnibVersoUrl, 'CNIB Verso'),
                        icon: const Icon(Icons.open_in_new, color: Colors.blue),
                        label: Text(
                          'Voir le verso CNIB enregistré',
                          style: TextStyle(color: Colors.blue[900]),
                        ),
                      ),
                    ),
