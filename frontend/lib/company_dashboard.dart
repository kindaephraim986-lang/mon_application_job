...
class _CompanyDashboardState extends State<CompanyDashboard> {
  ...
  Future<void> _loadAvailableDomains() async {
    try {
      ...
      setState(() {
        _availableDomains = list.isEmpty ? [] : list;
        _selectedDomaine = entrepriseData['domaine'] ?? (_availableDomains.isNotEmpty ? _availableDomains.first : null);
      });
    } catch (e) {
      setState(() {
        _availableDomains = [];
        _selectedDomaine = entrepriseData['domaine'];
      });
    }
  }
  ...
  void _publierOffre() async {
    ...
    final resp = await ApiService.createOffer(
      ...
      domaine: _selectedDomaine ?? entrepriseData['domaine'],
          ...
          if (_availableDomains.isNotEmpty)
            Padding(
              ...
              child: DropdownButtonFormField<String>(
                ...
                decoration: const InputDecoration(labelText: "Domaine / Filière"),
                items: _availableDomains.map((d) => DropdownMenuItem(value: d, child: Text(d))).toList(),
                onChanged: (v) => setState(() => _selectedDomaine = v),
              ),
            ),
